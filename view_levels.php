				...
				<div class="span7" id="">  
					  ...
					  <?php	
					   ...
					   <!-- Modal -->
						...
						if(isset($_POST['color'])){
							...
							?>
							<a button type='button' class="btn btn-primary" href="levels.php">Назад</a>
									...
									<div class="table-responsive">
  									<table cellpadding="0" cellspacing="0" border="0" class="table" id="example">
										...
										<tbody>
													<?php 
						$id=$_GET['id'];
						$query=mysql_query("SELECT tenant_id,fname,mname,lname,address,contact,color FROM tenant WHERE level = $id")or die(mysql_error());
						while($row=mysql_fetch_array($query)){
						$id = $row['tenant_id'];
						$color = $row['color'];
					if ($color == "")
					{
						...
						?>
												...
												<td style="color:<?php echo $row['color']; ?>"><center><a href="user.php?id=<?php echo $row['tenant_id']; ?>" style="color:<?php echo $color; ?>"><?php echo $row['fname']."&nbsp;".$row['mname']."&nbsp;".$row['lname']?></a></center></td><? $quer = mysql_query("select * from follow where tenant_id='$id' AND admin_id='$idadm' ")or die(mysql_error());
