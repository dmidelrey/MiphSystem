...
<br>
<br><form class="form-horizontal" role="form" method="post">
<div class="form-group">
<label class="col-md-5 control-label" for="log">Задача:</label>  
<div class="col-md-3">
<select id="zad_id" name="zad_id" class="form-control" >
<?
$id = $_GET['id'];
...
{
	$level=$zad['level'];
	$sql_add = "SELECT * FROM tenant WHERE level='$level'";
	$resultu = mysql_query($sql_add);
	$rows=mysql_num_rows($resultu);
	$kek=1;
	while($kek<=$zad['kolvo'])
	{?>
	<option value="<?php echo $kek; ?>">№<?php echo $kek; ?></option>	
	<?$kek=$kek+1;}
}
?>
</select></div></div>
...
<?php 
						...
						while($rec=mysql_fetch_array($query)){
							$id1=$rec['tenant_id']; ?>
							<div class="form-group">
							  <label class="col-md-5 control-label" for="rental"><?php echo $rec['fname']; ?> <?php echo $rec['mname']; ?> <?php echo $rec['lname']; ?></label>
							  <div class="col-md-3">
							  <input type='hidden' name='rows' value=<?php echo $rows; ?>>
							<input name='ocenka<?php echo $rec['tenant_id']; ?>' type="text" class="form-control input-md" placeholder="Балл за задачу">
								</div></div>
						<? }?>
