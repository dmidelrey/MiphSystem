				...
				<center><h4><strong>Таблица знаний</strong><h4></center>
                       ...
                       ?>	
						...
						if(isset($_POST['color'])){
							...
							?>
							<a button type='button' class="btn btn-danger" href="waterminus.php">Уменьшить знания</a>
						...
						$query=mysql_query("SELECT * FROM tenant LEFT JOIN room
													ON tenant.room_id=room.room_id ORDER BY level")or die(mysql_error());
					...
					if ($color == "")
					{
						...
						<td width="30">
												...
												<td style="color:<?php echo $rec['color']; ?>"><center><h5><a href="view_levels.php?id=<?php echo $rec['level']; ?>" style="color:<?php echo $color; ?>"><?php echo $rec['level']; ?></h5></center></td>
						<td style="color:<?php echo $rec['color']; ?>"><center><h5><a href="view_tenant.php?id=<?php echo $rec['room_id']; ?>" style="color:<?php echo $color; ?>"><?php echo $rec['room_name']; ?></h5></center></td>
						<? $quer = mysql_query("select * from follow where tenant_id='$tenant' AND admin_id='$idadm' ")or die(mysql_error());
						$count = mysql_num_rows($quer); 
						if ($count > 0)
						{?>
						<td width="100"><center><a href="user.php?id=<?php echo $rec['tenant_id']; ?>" style="color:<?php echo $color; ?>"><i class="fa fa-fw fa-star"></i></center></td>
						<? }
						else { ?>
						<td></td>
						<? } ?>
						
						<td style="color:<?php echo $rec['color']; ?>"><center><h5><a href="user.php?id=<?php echo $rec['tenant_id']; ?>" style="color:<?php echo $color; ?>"><?php echo $rec['fname']."&nbsp;".$rec['mname']."&nbsp;".$rec['lname']?></a></h5></center></td>
